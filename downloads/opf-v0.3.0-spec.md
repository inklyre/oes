...
| `opf_version` | string | yes | Any `0.3.x` patch — see Versioning & Conformance on the docs site. |
...
| `questions` | object[] | yes | Minimum 1, ordered. Each entry is a direct reference, a pool, or a multi-part group. See below. |
...
`questions[]` entries — group (a multi-part question), identified by
having `parts`:

| Field | Type | Required | Description |
|---|---|---|---|
| `id` | string | yes | Identifies the group within the set. |
| `parts` | object[] | yes | Minimum 1. Ordered sub-entries presented together under one heading, as a past paper's Q3 (a)/(b)/(c). Each part is an ordinary entry, so parts may be of different question types, and a part may itself have `parts`. |
| `label` | string | no | Display label — `"3"`, `"a"`, `"ii"`. Derived as a, b, c… from position when absent. Display only; `id` remains the identifier. |
| `title` | string | no | Optional display title for the group. |
| `answer_any` | integer | no | "Answer any N of these parts." MUST be <= the number of parts. |

A group has no `points` of its own; its total is the sum of its parts.
Parts MUST NOT be reordered, and a consumer shuffling a set MUST move the
group as a unit — later parts routinely refer to earlier ones by label.

`answer_any` is not a pool's `select`: a pool picks at random and the
learner never sees the rest, while `answer_any` shows every part and lets
the learner choose. It is advisory intent a platform MAY enforce.

A shared stem belongs on each part's own question via OQF's `stimulus`,
not on the group — that is what keeps every part independently
resolvable. A consumer SHOULD render it once above the parts.

Direct references, pools, and groups can be freely mixed and ordered in
one `questions[]` list.
...
Full schema: `schemas/opf/v0.3.0/set.schema.json`. Question content is
validated separately against `schemas/oqf/v0.2.0/question.schema.json` —
...
Bump `opf_version` to `"0.3.0"`. Nothing else: v0.3.0 is purely additive,
and every existing direct reference and pool validates unchanged.

The version still moves because a consumer built for `0.2.x` does not
know what `parts` means. Under the versioning policy it refuses a
`0.3.0` document outright rather than silently rendering a multi-part
question as one empty entry.
