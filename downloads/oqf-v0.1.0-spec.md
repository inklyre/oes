...
| `oqf_version` | string | yes | Any `0.1.x` patch — see Versioning & Conformance on the docs site. |
...
`image: string` (yes, relative path), `labels: [{id, answer, position:{x,y}}]`
...
```
{stimulus-root}/
├── stimulus.json     {oqf_version, id, title?, authors?, license?, tags?}
├── stimulus.md        (optional — the shared prompt, pure Markdown)
└── assets/             (optional)
```
...
Full schemas: `schemas/oqf/v0.1.0/question.schema.json` and
`schemas/oqf/v0.1.0/stimulus.schema.json` — the canonical, machine-readable
